...
The main objective of this project was to develop CarotidCheck, a cloud-integrated software solution that compares two deep learning architectures—Vision Transformer (ViT) and Attention U-Net—for carotid artery segmentation. The best-performing model was deployed to provide **objective community-level carotid ultrasound screening** (IMT-based risk stratification and conditional NASCET stenosis) and to **strengthen referral visibility** for higher-risk cases—not to diagnose acute stroke or replace FAST.
