...
The **full capstone report** is normally submitted as a **Word/PDF** package with the institutional **title page**, **declaration**, **certification**, automatic **Table of Contents**, **lists of figures/tables/acronyms**, and **page numbers**. This Markdown file is the **repository master copy** of the chapter text—copy sections into Word for final layout, or edit here first and paste into your template. If the faculty template’s declaration still reads **“Project Proposal”**, replace it with **capstone project report** (or your programme’s exact term) for the final submission.
...
Traditionally, interventions relied on hospital-based imaging (CT/MRI) and public awareness campaigns such as the FAST protocol (Face, Arm, Speech, Time), which targets **recognition of suspected acute stroke** (sudden focal symptoms) and emergency action—not routine carotid imaging. While valuable, acute pathways and hospital imaging remain hard to access in rural areas where neurologists are scarce and equipment is centralized in urban hospitals (Liu et al., 2024). Separately, **community vascular screening** from ultrasound lacks standardized, objective measurement at the edge. Cloud-integrated computer vision offers a complementary path: **image-based triage** from carotid scans, not a replacement for FAST in the acute setting. By deploying high-performance models such as Vision Transformers (ViT) and Attention U-Net, along with preprocessing pipelines using padding, resizing, and Albumentations-based augmentation, smartphones can now support objective biomarker identification from carotid ultrasound images.
...
The primary gap addressed by this research is the absence of an integrated, **image-driven vascular screening** pipeline that links community health posts to hospitals (**IMT-focused triage and referral**), alongside—not instead of—acute pathways such as FAST. CarotidCheck addresses this gap by automating carotid artery segmentation and IMT measurement when segmentation is reliable, supporting timely referral and follow-up; **non-measurement** is reported explicitly rather than imputing risk.

## 1.3 Project’s main objective

The main objective was to deliver **CarotidCheck**: a **machine learning–driven** carotid segmentation and IMT pipeline (**ViT vs. Attention U‑Net** on Momot (2022)), deployed inside a **connected clinical system** (field → cloud → hospital) with **evaluation and responsible deployment**—including explicit **Unknown** outcomes when measurements are unreliable, and a path toward **ethics-approved** field validation.

### 1.3.1 Specific objectives

1. **Machine learning (segmentation and IMT)**  
   Develop and compare **Vision Transformer** vs **Attention U‑Net** on the **Momot (2022)** carotid ultrasound dataset; optimise **Dice**, **IoU**, and **F1**; select the best model for deployment (target **Dice > 0.85** on the validation setup used in this project).

2. **Connected clinical system (field → cloud → hospital)**  
   Build a **FastAPI** backend with **JWT** authentication and **PostgreSQL**; a **Flutter** app for Community Health Workers (CHWs) for capture and results; and a **React + Vite** clinician dashboard with **high-risk referral visibility** and **in-app notifications** (bell/badge, API-polled).

3. **Evaluation and responsible deployment**  
   Validate with **automated tests**, **API health** and **latency** observability, and a **simulated workflow**; **plan ethics-approved field validation**; enforce **quality gates** (**Unknown** when segmentation or plausibility checks indicate unreliable measurement) and **privacy-aligned** design (e.g. UUID-linked storage, consent-oriented workflows as implemented in the system).

## 1.4 Research questions

**Primary research question:** Can **machine learning–based** carotid ultrasound analysis support **earlier stroke-risk identification** and **timely referral** in Rwanda?

**How this project addresses it:** The question is operationalised through **CarotidCheck**: (i) **compare Vision Transformer (ViT) and Attention U-Net** on the Momot (2022) dataset and **select the best checkpoint by validation Dice** (target Dice > 0.85); (ii) deploy that model in a **cloud pipeline** (FastAPI, PostgreSQL) with a **Flutter** client for CHWs and a **React + Vite** clinician dashboard for **referral visibility** and notifications; (iii) evaluate with **automated tests**, **health/latency** endpoints, and a **simulated workflow**, with **Unknown** outcomes when segmentation is unreliable—recognising that **“earlier”** and **“timely referral”** here mean **screening-oriented triage and faster clinician awareness**, not replacing emergency acute-stroke pathways such as **FAST**.
...
The successful implementation of CarotidCheck strengthens **carotid ultrasound triage** in Rwanda by complementing **non-imaging** workflows with **objective, AI-driven IMT** (and stenosis when geometry allows). By automating the measurement of carotid Intima-Media Thickness (IMT) where the segmentation is reliable, the software equips community health posts with **screening-oriented** capabilities that were often concentrated in specialized hospitals in Kigali. This approach supports Rwanda's Ministry of Health 4×4 Reform (Rwanda Ministry of Health, 2023), which seeks to empower the health workforce with digital tools to deliver specialized care at the community level.
...
The research and systems reviewed in this chapter highlight a critical gap in **community-level, image-driven vascular assessment**, particularly in rural and resource-limited settings. Tools such as PINGS and the Stroke Riskometer™ rely on **self-reported data** or **physiological / lifestyle** inputs—and **symptom recognition campaigns (e.g. FAST)** focus on **acute** focal neurological signs to trigger emergency care. Those pathways are vital but **do not automate carotid ultrasound** or standardize **IMT** at the CHW edge; they therefore leave a gap for **screening-time** objective imaging biomarkers linked to referral.
...
The literature thus supports the need for a fast, objective, and connected **carotid ultrasound triage** system tailored to decentralised healthcare environments. CarotidCheck addresses this need by integrating AI-driven ultrasound analysis with a cloud-based architecture capable of **screening-oriented decision support**. By surfacing **arterial wall metrics (IMT)** and **NASCET stenosis when computable**, and by routing **high-risk referrals** to clinicians, the system adds a **prevention-oriented screening channel** alongside—not in place of—**acute symptom recognition (FAST)** and emergency care.
...
**Primary research question** (*Can machine learning–based carotid ultrasound analysis support earlier stroke-risk identification and timely referral in Rwanda?*): This implementation **supports** that aim in a **technical and workflow** sense: **ViT** vs **Attention U-Net** were compared (best checkpoint **Attention U-Net** on validation Dice/IoU), deployed behind **FastAPI** with **PostgreSQL**, Flutter, and a clinician dashboard with **referral visibility**. **Earlier identification** is framed as **community IMT-based screening** with explicit **Unknown** when unreliable; **timely referral** is framed as **dashboard notification** of high-risk cases—not a claim to close the full **72-hour** care-seeking gap alone. **End-to-end latency** on free-tier CPU can **exceed** the 5 s design target (§5.1.2); stronger hosting or GPU would be needed for stricter latency goals.
...
- **Lumen / wall segmentation:** Improve model robustness so lumen geometry for **NASCET** is available on a larger share of scans (wall detection, probe angle, and speckle variability).  
